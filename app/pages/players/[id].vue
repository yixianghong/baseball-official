...
<template>
  <div class="container-content py-10 md:py-14">
    ...
    <div v-else-if="player" class="grid gap-8 md:grid-cols-[minmax(0,18rem)_1fr]">
      <!-- ── 個人卡片 ─────────────────────────────────────────── -->
      <aside>
        <div class="overflow-hidden rounded-2xl border border-border bg-surface-raised">
          <div class="aspect-4/5 bg-ink">
            <img
            ...
            <div v-else class="field-pattern flex size-full items-center justify-center">
              <span class="text-7xl font-black text-white/70 tabular-nums">
                {{ player.number || '—' }}
              </span>
            </div>
          </div>
          ...
          <div class="space-y-4 p-5">
            ...
            <dl class="space-y-3 text-fluid-sm">
              <div class="flex justify-between gap-4">
                <dt class="text-content-muted">守備位置</dt>
                <dd class="text-right font-medium">
                  {{ player.positions.map((p) => POSITION_LABELS[p]).join('／') }}
                </dd>
              </div>
              <div class="flex justify-between gap-4">
                <dt class="text-content-muted">投打習慣</dt>
                <dd class="font-medium">{{ handText }}</dd>
              </div>
              <div v-if="player.joinedYear" class="flex justify-between gap-4">
                <dt class="text-content-muted">加入年份</dt>
                <dd class="font-medium tabular-nums">{{ player.joinedYear }}</dd>
              </div>
              <div class="flex justify-between gap-4">
                <dt class="text-content-muted">狀態</dt>
                <dd>
