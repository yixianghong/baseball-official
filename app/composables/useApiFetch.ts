 ...
 * const { data: items, pending, error } = useApiFetch<Game[]>('/games')
 * </script>
 *
 * <template>
 ...
 * const { data, pagination } = useApiFetch<Game[]>('/games', {
 ...
 * const { data: item } = useApiFetch<Game>(() => `/games/${route.params.id}`)
 * ```
 *
 * @example 讀取分頁資訊
 * ```ts
 * const { data, pagination } = useApiFetch<Game[]>('/games', { query: { page } })
 * // pagination.value → { page, pageSize, total, totalPages, hasNext }
 * ```
 *
 * @example 不要在 SSR 執行（例如需要瀏覽器 API 的資料）
 * ```ts
 * const { data } = useApiFetch('/games', { server: false, lazy: true })
 * ```
 *
 * @example 手動重新整理
 * ```ts
 * const { data, refresh } = useApiFetch('/games')
 * // 使用者按下重新整理按鈕
 * await refresh()
 * ```
 *
...
export function useApiFetch<T>(
   ...
   *
   * ```ts
   * const { data } = useApiFetch('/games')          // 不等待，用 pending 控制畫面
   * const { data } = await useApiFetch('/games')    // 等資料到齊才繼續往下
   * ```
   *
