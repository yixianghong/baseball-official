 ...
 * const { get } = useApi()
 * const items = await get('/games')
 *
 * // ✅ 正確：初始資料請用方式 A
 * const { data: items } = useApiFetch('/games')
 * ```
 *
 ...
 *   try {
 *     const created = await post<Game>('/admin/games', form)
 ...
 *
 * const { data, error } = await attempt(() => post('/admin/games', form))
 ...
 * ```ts
 * const { data: items, refresh } = useApiFetch<Game[]>('/games')
 * const { del, loading: deleting } = useApi()
 *
 * async function remove(id: string) {
 *   await del(`/admin/games/${id}`)
 ...
 *   const next = await get<Game[]>('/games', { query: { page: ++page.value } })
