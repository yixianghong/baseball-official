 ...
 *
 * ## 兩種呈現
 * - `compact`（導覽列）：只有鈴鐺與開關，沒有說明文字。開不了的狀態直接不顯示 ——
 *   導覽列上放一個永遠按不動的開關只是噪音。
 * - 完整（手機選單）：帶文字說明，並且會把「為什麼開不了」講出來。
 *   iOS 必須先加入主畫面這件事只能靠文字說明，沒有程式可以代勞。
 *
...
<template>
  <div v-if="!hidden" :class="compact ? 'flex items-center' : 'px-5 py-3'">
