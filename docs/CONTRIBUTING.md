...
### 1. 前端不直連 Firebase 或任何外部服務

所有資料存取與外部呼叫都必須經過 `server/`。前端只能打 `/api/*`，
**不要安裝或引入 Firebase 的前端 SDK**。

前端一旦能直接連 Firestore，就必須把存取規則寫進 Firebase 安全規則，
憑證也得交給瀏覽器保管 —— 這個專案選擇讓 BFF 用 service account 存取，
瀏覽器端完全沒有任何憑證可偷。

CSP 的 `connect-src 'self'` 會在瀏覽器層擋下違規的嘗試，
...
`useApiFetch` 把 `useFetch` 包了一層，這會帶來一個**必然的**副作用：
Nuxt 原生依「呼叫位置」產生 key，包起來之後整個專案只剩一個呼叫點，
key 退化成「只看網址」—— **兩個讀同一支端點的頁面共用同一筆快取**。

而 Nuxt 只在「沒有任何元件還在用它」時才清快取，換頁時新頁面**先掛載、
舊頁面才卸載**，參照數永遠不會歸零。結果是：在那兩頁之間來回走，
Nuxt 直接沿用舊資料，**一次請求都不會發**。後台改完資料走去另一頁再回來，
畫面就變回改之前的內容 —— 而且表單被舊資料灌回去，再存一次會真的蓋掉。

所以 `revalidateOnEnter` **預設是 `true`**：進到頁面時，如果這次呼叫一個
請求都沒發（代表沿用了別頁的快取），就補抓一次並擋在 `await` 前面。
這讓行為回到「原生 useFetch 各頁各自的快取」，**新專案不必為每支端點想這件事**。
...
| -------------------- | ------------------------- |
| 這一頁是唯一的讀取者 | 0（本來就會發）           |
| 整頁載入／hydration  | 0（用 SSR payload）       |
| 沿用了別頁的快取     | 1（就是原本會出錯的那次） |

**只有一種情況該關掉**：掛在 layout 上、每頁都要、而且幾乎不會變的資料
（本專案只有 `useSiteSettings()`）。那種資料的快取不會被清，開著等於每次
換頁都多一個阻塞的來回。關之前先問：**這份資料在後台被改過之後，使用者
會不會在不重新整理的情況下看到舊的？** 會的話就不該關。

參考實作：`app/composables/api/useGamesApi.ts`、`app/composables/api/useAuth.ts`。
判斷邏輯與實測見 `app/composables/useApiFetch.ts`，
回歸測試在 `tests/nuxt/useApiFetch.test.ts`。
...
`server/api/**` 只呼叫 `server/repositories/` 的函式，不直接使用 Firestore SDK。

這是第 3 條（頁面只呼叫 composable）往下延伸一層：資料來源要換
（Firestore → Postgres、或加一層快取）時只改那個資料夾，端點一行都不用動。
記憶體模式與真實資料庫的切換也藏在那一層，端點完全不需要知道。

另外：登入頁路徑不要寫死，改讀 `runtimeConfig.public.loginPath`。
...
| `app/`                          | 元件、composable、Pinia store                       | 任何 secret、Firebase SDK、直接呼叫外部服務     |
| `server/`                       | BFF 邏輯、Firestore 存取、外部服務呼叫              | 瀏覽器 API（`window`、`document`）              |
