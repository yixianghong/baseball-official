 ...
 *
 * ## 核心原則：Firebase token 永遠不離開 Node 層
 * 前端 Firebase SDK 的做法是讓瀏覽器自己保管 idToken，任何一個 XSS 就能把它
 * 偷走。這個專案改成：
 *
 * ```
 * 瀏覽器 ──(帳號密碼)──> Node BFF ──> Firebase Identity Toolkit
 *   <──(httpOnly 加密 cookie)──┘  ↑ idToken 只存在這一段
 * ```
 *
...
export interface AppSessionData {
  ...
  user?: PublicUser
  /**
   * Firebase 登入時取得的 idToken。
   *
   * 本專案的資料存取走 Admin SDK（service account），不需要使用者的 token，
   * 所以這個欄位平常沒有用途 —— 保留它是為了日後若要改用「以使用者身分
   * 存取 Firestore」時有地方放，且能確保它從來沒有離開過 Node 層。
   */
  accessToken?: string
  /** Firebase 的 refresh token，用於日後續期。 */
  refreshToken?: string
 ...
 * const { user, idToken } = await signInWithPassword(event, email, password)
 * await createUserSession(event, { user, accessToken: idToken })
...
export async function createUserSession(
  ...
  payload: {
    user: PublicUser
    /** 選填：本專案不依賴它，見 `AppSessionData.accessToken` 的說明。 */
    accessToken?: string
    refreshToken?: string
