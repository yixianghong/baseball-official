...
比賽頁會顯示當天的天氣預報，資料來自中央氣象署開放資料平臺。

申請授權碼（免費）：https://opendata.cwa.gov.tw/user/authkey ，填進 `NUXT_CWA_API_KEY`。
留空時整個功能關閉，網站其餘部分不受影響。

### 兩個限制

**預報只有一週。** 賽程常常幾週前就排好，那些場次查不到 —— 前台會明講
「預報只到 7 天後」，用說明的語氣而不是錯誤的語氣。超出範圍的日期根本不發請求。

**地點要自己選。** 氣象署的預報要指定縣市，而場地欄位是自由輸入的文字
（「新莊新月橋」「大漢 A 球場」），硬猜會顯示成別的縣市的天氣，比不顯示更糟。
所以後台每場選縣市，預設帶入網站設定的「主場縣市」。

### 資料集的坑（都是實際打過 API 確認的）

用的是 **`F-D0047-091`**。它掛在「鄉鎮天氣預報」底下，但實際回傳 22 個縣市，
正好對應後台的選單。以下每一條猜錯都會變成「沒有錯誤、但永遠查不到資料」：

|               |                                                                                       |
| ------------- | ------------------------------------------------------------------------------------- |
| `F-C0032-005` | **不存在**，回 404                                                                    |
| 外層結構      | `records.Locations[].Location[]`，**大寫開頭**（舊資料集是小寫 `records.location[]`） |
| `ElementName` | **中文**：`天氣現象`、`最高溫度`、`12小時降雨機率`，不是 `Wx`／`MaxT`／`PoP`          |
| 值的鍵名      | `Weather`、`MaxTemperature`、`MinTemperature`、`ProbabilityOfPrecipitation`           |
| 篩選參數      | **`LocationName`（大寫 L）**。小寫會被安靜地無視，回傳全部 22 縣市（636KB vs 30KB）   |
| 降雨機率      | 四天以後是 `"-"` 而不是數字，那時不顯示這一行                                         |

時段是 12 小時一段（白天 06–18、夜間 18–06），程式挑**包含開賽時間**的那一段 ——
把夜間低溫混進來平均，顯示出來會是一個當天根本不會出現的溫度區間。

## 分享出賽名單

出賽名單圖卡下方有兩顆按鈕，各自做一件明確的事：

| 按鈕         | 行為                                                                                                          |
| ------------ | ------------------------------------------------------------------------------------------------------------- |
| **下載圖片** | 把圖卡畫成 PNG 存檔。不依賴任何瀏覽器 API，永遠有效                                                           |
| **分享**     | 叫出系統分享選單並帶上 PNG（LINE、相簿…）。瀏覽器不支援檔案分享時退回分享／複製本頁網址，並明白說出發生了什麼 |

不做成「一顆聰明的按鈕自己判斷」是因為使用者按下去之前不知道會發生什麼 ——
想存檔的人被叫出分享選單，想貼到群組的人拿到一個檔案。

### 隊徽為什麼要繞一圈

canvas 只要畫進**沒有 CORS 標頭**的跨網域圖片就會被污染，`toBlob()` 直接拋
SecurityError。`storage.googleapis.com` 上的隊徽實測沒有這個標頭（預設的
bucket 沒設 CORS），所以截圖時改從 `/api/media/remote` 取回。

那支端點沒有登入保護，**白名單就是唯一的防線**：只接受 https、主機名稱必須
完全等於已知的 Storage 網域、路徑必須落在本專案自己的 bucket 底下。
少了任何一條，它就變成「任何人都能叫我們的伺服器去打任意網址」。

另一條路是去 bucket 設 CORS 規則，但那要改動線上資源，而且每換一個 bucket
就得再設一次。
...
| `/schedule`     | 近期賽程，由近到遠                                                                                                                     |
| `/results`      | 比賽結果。**月曆**呈現，點日期直接進那場比賽；下方列出當月場次。可依年度篩選並顯示總戰績                                               |
| `/games/[id]`   | **未開打** → 預計出席 + 出賽名單；**已結束** → 計分板 + 出賽名單 + 投手。名單是一張轉播風格的圖卡（先發打序 + 候補），可一鍵分享成 PNG |
