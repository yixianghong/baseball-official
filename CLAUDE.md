...
會裁切，看起來完全正常，錄出來卻是直式影片。症狀是使用者回報「我明明轉成
橫的，它還說影像是直的」：警告沒說錯，是串流真的沒變。

解法不是只重讀一次設定，而是**重新 `getUserMedia()`** 讓 track 以新方向重新
協商（`onOrientationChange`，延遲 400ms 等版面穩定，因為 `orientationchange`
會在旋轉完成前就觸發）。**錄影中絕對不能重開** —— 那會讓 `MediaRecorder` 的
來源消失，那一段就毀了，所以錄影中只顯示警告並要求先結束這一段。

`portraitVideo`（`width < height`）偵測到就在預覽上壓紅色警告，並把實際的
數字印出來 —— 有數字才分得出是「邏輯寫反」還是「串流真的是直的」。
