...
**⚠️ 判斷「手機橫放」要用 `field-mode:` 而不是 `landscape:`。**
桌機瀏覽器**永遠是橫向的**，所以 `landscape:` 會把桌機也算進去 —— 錄影頁
因此在電腦上測試時把上傳錯誤的詳細清單整個藏起來，症狀是「顯示 2 段失敗
但看不到原因」。`field-mode` 是 `(orientation: landscape) and (max-height: 600px)`，
定義在 `main.css`。踩過這一次。

**橫向是另一套版面，但仍然是上下排列（不左右並排）。** 實測直向的垂直堆疊
有 855px 高，橫向可用高度只有 320px 上下 —— 照搬過去錄影鈕會在畫面外 459px。
球場模式的做法是：預覽用 `34dvh` 的高度上限收斂（橫向不能用 `w-full`，
16:9 會算出比視窗還高），**局數與上下半併成一行**（橫向有的是寬度、缺的是
高度，分兩排的話上下半會被擠出畫面），小標與說明文字隱藏，
**錄影鈕放在捲動區外面**。高度用 `dvh` 不用 `vh`
