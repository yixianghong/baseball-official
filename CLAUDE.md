...
- 計分板存「我隊／對手」而非「上下半局」，誰先攻由 `homeAway` 推導。
  **要排成「上面那列是先攻」的地方一律走 `scoreboardSides()`**（它直接由
  `battingSide()` 推導，所以「客隊先攻」只寫在一個地方）。後台的計分板編輯器
  原本寫死「我隊在上」，於是主場的比賽在後台是「我隊／對手」、前台是
  「對手／我隊」—— 而計分板正是拿來核對的東西，順序相反看起來就像資料被改過。
  現在前台計分板、後台編輯器、首頁比分橫幅都呼叫同一支
  （`tests/nuxt/scoreboard-editor.test.ts` 直接比對前後台排出來的順序）
...
- **投手紀錄與打擊紀錄的成績是純文字**（`note`，例如「6 局 2 失分」
  「4 打數 2 安打 1 打點」），**系統不解析也不加總**。理由見下方「賽後紀錄」
- **候補名單是推導的，不存欄位**（`deriveBench()`：出席 `yes`、且**不在打線
  也不在投手紀錄上**）。多存一份就多一個會不同步的東西 —— 排進打線卻忘了從
  候補移除，同一個人就會既先發又候補。比對以 `playerId` 為主、姓名為輔
  （打線與投手紀錄都允許沒有 `playerId`）。
  ⚠️ **「沒上場」不等於「不在打線上」**：DH 制下先發投手不打擊，所以他根本
  不會出現在打線裡 —— 只比對打線的話，出賽名單圖卡上會同時印出
  「先發投手 #28」與「候補 #28」，同一個人既是最先發的又是坐板凳的（線上
  發生過）。中繼與終結是賽後才登錄的，賽前不影響，賽後排除也對：他們上場
  投球了。`tests/unit/game-logic.test.ts` 的 `deriveBench` 那組守著這三條。
...
- **橫跨畫面下緣的 `fixed` 容器會把底下的按鈕全部吃掉。** toast 的容器
  （`UiToastHost`）平常是空的，但它是一個 `inset-x-0 bottom-0` 的 fixed 元素 ——
  沒有 `pointer-events-none` 的話，整個畫面下緣的按鈕都點不到，而畫面上
  **完全看不出原因**（看起來就是「這顆按鈕壞了」）。每一則提示自己要補
  `pointer-events-auto`，否則連關閉鈕也點不到。`tests/nuxt/toast.test.ts` 守著。
  它的 `z-index` 還要高於 `AdminRowMenu`（遮罩 40、選單 50），
  不然提示會被那層全螢幕遮罩壓住，「重試」按鈕一樣點不到。
...
- **呼叫外部服務時一定要自己接住錯誤，不能讓它往上冒。**
  `createExternalClient` 依**上游的 HTTP 狀態**丟出我們自己的錯誤碼
  （400 → `BAD_REQUEST`、401 → `UNAUTHORIZED`），所以 Google 回 400
  會變成前台的「請求格式不正確」、回 401 會變成「**請先登入**」——
  兩個都把人指向完全錯誤的方向，而真正的原因一個字都沒出現。
  實際踩過：YouTube 的 refresh token 被截斷，線上回 400，查了一輪才發現
  和「請求格式」無關。上游真正說了什麼在 `err.cause.data` 裡，
  要自己挖出來記進日誌（見 `server/utils/youtube.ts` 的 `describeUpstream()`）。
...
後台的編輯畫面**沒有儲存按鈕**（`useAutosave`）：出席登記這種「點十幾下、每下
都是一個獨立決定」的操作，按鈕只是多一個會忘記的步驟。代價是「以為存好了，
其實沒有」變成最危險的失敗模式，所以狀態必須看得見。

**狀態用 toast 呈現，不用釘在表單上方的一行字。** 那一行原本在頁首，而後台的
單場賽事頁有四個分頁、捲起來很長 —— 使用者在填計分板時看不到頁首，而那正是
最想確認「到底存進去了沒有」的時候。toast 固定在畫面右下角（手機置中），
和捲到哪裡無關。

提示由 `useAutosave` **自己送出**，頁面不必也不該再接一次：
...
- **`pending` 與 `saving` 刻意不提示** —— 它們通常只持續幾百毫秒，替它們發提示
  的結果是每次打字停頓畫面都閃一下。「還沒存完就離開」不靠使用者看到提示來擋，
  而是換頁時 `flush()`、關閉分頁時 `beforeunload`。
- **所有提示共用同一個 key**（`'autosave'`），所以連續儲存是同一則訊息**就地
  更新**。沒有 key 的話，改十次就有十則一模一樣的「已自動儲存」從畫面下方長出來。
- **失敗的提示可以關掉。** 留一則關不掉的紅色訊息並不會讓資料更安全 ——
  真正的防線是狀態還在（下一次變更會再試一次）以及 `beforeunload` 的提醒。

⚠️ **子元件在載入後補資料，會被自動儲存當成使用者的變更。** 出席編輯器為了
「進到分頁就看得到隊員」會把現役名單補成「未回覆」，而它掛載的時機在父元件
`markAsSaved()` **之後** —— 於是光是打開一場還沒登記出席的比賽，就會寫一次
資料庫（實測 11 筆）。狀態改用 toast 呈現之後這件事才浮出來：一開頁面就跳
「已自動儲存」。修法是把補名單抽成純函式 `mergeAttendanceWithRoster()`，
**父元件灌資料時就先補好**，基準一開始就含那份名單，編輯器算出同一個結果、
發現沒變就不寫回。⚠️ 不要用「等一個 `nextTick` 再重新取基準」——
實測擋不住（子元件的掛載不在那個時機裡），而且那等於把正確性綁在渲染順序上。
**任何會在載入後自動補資料的子元件都要用同樣的方式處理。**

⚠️ **toast 不適用於需要使用者做決定的事**，因為它會自己消失：刪除確認走
`AdminDeleteButton` 的兩段式按鈕，欄位錯誤顯示在那一格旁邊（`UiBaseInput` 的
`error`），整頁載入失敗用 `UiBaseError` 佔住版面。`useToast` 的說明裡有完整的界線。

### 比賽進行中的自動更新

比賽頁（`/games/[id]`）在 `status: 'live'` 時每 `NUXT_PUBLIC_GAME_POLL_MS`
（預設 120 秒）重抓一次資料，讓計分板、比數與影片片段跟著後台更新 ——
這一頁的 HTML 走 CDN 快取，沒有輪詢的話要手動重新整理才看得到變化。

三個「不要浪費請求」的條件：**只有進行中才輪詢**（已結束的比賽不會再變，
而且狀態翻成 `finished` 的那一次請求會抓到最終結果然後自己停掉）、
**分頁在背景就暫停**、間隔設 `0` 就整個關掉。

刻意**不替 `/api/games/[id]` 加 CDN 快取**：後台讀的是同一支端點，
替它加上共用快取會讓「我明明存檔了」變成客訴（見「部署」章節）。
以這個站的觀看規模，120 秒一次的原站請求微不足道。

⚠️ 那個 `watchEffect` 必須放在它用到的 `const` **之後** —— 它會立刻執行一次，
放前面會踩到暫時性死區（見上面的坑）。
...
兩份都在「賽事管理」分頁，形狀幾乎一樣：`{ playerId, name, number, note }`
（投手多一個 `role`：先發／中繼／終結）。

前台收在**同一個「本場紀錄」區塊**底下，各自用 `<h3>`（投手／打擊）分開 ——
它們是同一件事的兩面（這一場我們打得怎麼樣），做成兩個平行的 `<h2>` 會讓
頁面上出現兩個看起來不相干的區塊。兩份都空的時候整個區塊不出現。

**兩份上下排，不左右並排**：投手通常一到三個人、打者九到十二個，
並排會變成左欄兩列、右欄十二列，中間空一大塊。

**⚠️ `note` 是純文字，系統不解析也不加總。** 裡面寫的是「4 打數 2 安打 1 打點」
這種句子。兩個理由：

- **欄位越多，實際發生的事不是「資料更完整」而是「整段沒人填」。** 拆成
  打數／安打／打點之後，一場要在手機上填 9～12 個人乘以好幾格，而登錄的人
  是賽後在停車場打的。
- **別讓它看起來像可以算的東西。** 有了數字欄位，下一步必然是打擊率、
  季賽累計、排行榜 —— 而那需要每一場都完整且正確地登錄，這個球隊沒有記錄員。
  一個少登三場的 `.412` 比沒有數字糟糕得多。真的要做季賽統計時，該補的是
  結構化欄位**加上「這場有沒有完整登錄」的旗標**，而不是回頭去解析這些句子。

`tests/unit/game-logic.test.ts` 的 `batterEntrySchema` 那組守著這件事。

**打擊紀錄和打線是兩份資料，不要合併。** 打線是**賽前**排的棒次（有 `order`
與守備位置，會印在出賽名單圖卡上），打擊紀錄是**賽後**登錄的成績，而且可能
包含代打、不含沒上場的先發。後台有一顆「從打線帶入」一次把人補進來
（只補還沒在名單上的，不覆蓋已經填好的備註 —— 那顆按鈕很可能被按第二次）。
...
一概不放：資料模型裡**沒有可以算的球員數據**（賽後的打擊紀錄是純文字，
見「賽後紀錄」），而名單會被截圖傳到群組，上面的每個數字都會被當真 ——
憑空生一個 `AVG .342` 比留白糟糕得多。這張卡是**賽前**的名單，
賽後才有的成績本來也不該出現在上面。
...
**先發投手也只有「#背號 名字」**，投手紀錄的 `note`（「6 局 2 失分」）曾經
印在名字下面，後來拿掉了 —— 那是**賽後**才填得出來的東西，而這張卡在**賽前**
就被截圖丟進群組了。混進來之後同一張卡在不同時間點長得不一樣，看到舊截圖的人
會以為名單被改過。成績去看「本場紀錄」。現在卡片上沒有任何賽後資訊。

⚠️ **卡片的寬度就是下載下來的 PNG 的寬度**（`useShareRoster()` 截的是
`cardRef` 本身）。所以 `/games/[id]` 的出賽名單區塊綁著 `max-w-xl` ——
讓它在桌機上撐滿容器會截出一張又扁又寬、貼進群組看不清楚的圖。
...
**錄影頁要讀 `game.clips`，不能只看這次的上傳佇列。** 原本「這一輪已錄」與
局數按鈕上的「已錄」標示只來自上傳佇列 —— 那是這次打開頁面之後的記憶體狀態，
於是**離開錄影頁再回來，已經傳上去的片段全部不見、局數回到第 1 局上半**，
看起來就像錄影資料被清掉了（資料其實一直在比賽文件上）。現在清單、按鈕標示與
接續位置都用三個來源的聯集：已上傳（`game.clips`）、這次傳的（佇列）、
裝置上沒傳的（IndexedDB）。接續位置（`resumePosition()`）看的是**比賽裡最後
面的那一格**而不是時間戳 —— 錄影本來就照比賽順序往前推。同一個半局這次又錄了
一段時，清單只顯示這次的那一筆（上傳成功就會以「第幾局的哪半局」覆蓋網站上的）。
...
**錄影頁只有一套版面，橫向靠捲動而不是靠壓縮。** 預覽永遠是滿寬的 16:9、
高度不設上限（橫向時它比整個視窗還高，那沒關係），其餘資訊使用者自己捲。
真正不能捲掉的只有錄影鈕，所以**那一顆用 `fixed inset-x-0 bottom-0`**，
外層再留 `pb-32` 給它，否則最後一段內容永遠被蓋住。
⚠️ **不能用 `sticky bottom-0`** —— sticky 不會把元素拉出容器範圍，而那顆按鈕
本來就在文件末端，所以在捲到底之前完全沒有作用（看起來像沒生效，其實是規格如此）。

曾經為橫向做過第二套版面（預覽限高 `34dvh`、局數與上下半併成一行、小標隱藏），
目的是「橫向不用捲」。**那個目標本身是錯的**：為了省下捲動，取景畫面被壓到只剩
三分之一屏高，而取景正是這一頁唯一不能將就的東西。連帶拿掉的還有一個
`field-mode` 的 custom variant（`(orientation: landscape) and (max-height: 600px)`）
—— 它的存在是因為 `landscape:` 會把**永遠橫向的桌機**也算進去，害錄影頁在電腦上
測試時把上傳錯誤的清單整個藏起來（症狀是「顯示 2 段失敗但看不到原因」）。
少一套版面就少一整類「只在某個視窗尺寸才出現」的 bug。

高度一律用 `dvh` 不用 `vh`（`100vh` 含會收起的工具列），左右要留
`env(safe-area-inset-left/right)`（橫向時瀏海吃的是側邊）。
**不能用程式鎖定方向** —— `screen.orientation.lock()` 要先進全螢幕，
而 iPhone 對非 video 元素的 Fullscreen API 長期不支援。

**⚠️ 轉動手機不會讓已經取得的 track 跟著轉。** 在直向開啟頁面拿到的串流就是
直的（1080×1920），轉成橫的之後**它還是直的** —— 而預覽因為 `object-cover`
會裁切，看起來完全正常，錄出來卻是直式影片。

解法不是重讀設定，而是**重新 `getUserMedia()`** 讓 track 以新方向重新協商
（`onOrientationChange`，延遲 400ms 等版面穩定，因為 `orientationchange` 會在
旋轉完成前就觸發）。**錄影中絕對不能重開** —— 那會讓 `MediaRecorder` 的來源
消失，那一段就毀了，所以錄到一半才轉向的話那一段維持原方向。

畫面上**刻意沒有方向相關的提示或警告**：方向既然會自己修正，再跳出「影像是
直的」只會讓人困惑（實際發生過 —— 使用者轉成橫的之後反而看到那句話，因為
當下串流還沒重新取得）。預覽右下角只留解析度數字，給人核對「真的拿到
1920×1080 嗎」。`getSettings()` 不是響應式的，轉動時要自己重讀
（`settingsVersion`）。

**上傳走「BFF 發短效權杖、瀏覽器直傳 YouTube」。** 一段 85～140 MB，經過
Cloud Run 就要吃記憶體與請求逾時、而且同一份資料走兩趟網路。BFF 只做兩件事：
用 refresh token 換 access token（`server/utils/youtube.ts`，有快取並提早
5 分鐘過期），以及把影片的可見度讀回來。CSP 的 `connect-src` 因此開了
`https://www.googleapis.com` 這一個來源。

**錄完先存到裝置、再排進上傳佇列，順序不能反。** 反過來的話上傳失敗就等於
那一段沒了 —— 而球場的行動網路上，失敗是常態。失敗的 Blob 會留在記憶體裡
（才重試得了），成功才釋放。

**自動上傳與人工補登走同一支端點。** 片段的身分是**「第幾局的哪半局」而不是
videoId** —— `addGameClip()` 以 `(inning, half)` 為鍵覆蓋，所以「重錄」與
「上傳失敗後自己傳再補登」是同一個操作。刻意**不加 `source: auto | manual`
欄位**：它不改變任何行為，存了只是多一個要維護的東西。

**缺口是推導的，不存欄位。** 一場有幾個半局 = 計分板局數 × 2，後台列出全部、
沒有影片的那幾格自然就是待補的。上傳失敗時不必往資料庫寫「這段沒上去」——
那個寫入本身也可能失敗。補登用 `parseYouTubeVideoId()` 解析貼上的網址
（要驗網域：取出的 ID 會變成前台的 iframe src）。

**⚠️ YouTube 頻道有「每天能上傳幾支」的額度，和 API 配額是兩回事。**
Google 沒公布數字，依頻道信譽浮動（實務約 15～50），觸頂要等 24 小時 ——
一場十四段很容易撞到。撞到之後**當天剩下的每一段都會撞同一面牆**，所以
`useClipUpload` 認出 `uploadLimitExceeded` 就整批停下（`limitReached`），
而不是讓七段各失敗七次。
...
**⚠️ 沒有任何參數可以把播放器上的標題列與 YouTube 標誌拿掉。**
`modestbranding` 在 2023/08/15 被停用，加了完全沒有效果。實測（無頭 Chrome，
390px）確認過三件事：

- **沒有真的播起來的播放器最擋畫面** —— 停在封面時，標題、頻道頭像、正中央的
  紅色播放鈕與「觀看平台：YouTube」幾乎蓋掉半個畫面；真的在播的時候那些
  **會自己隱藏**。所以 `autoplay=1` 不是為了省一次點擊，是為了讓畫面乾淨。
  它有效的前提是 **iframe 在使用者點了縮圖之後才建立**（那一次點擊會被算成
  播放的使用者操作）——「一進頁面就放十四個 iframe」除了慢，還會全部卡在
  封面狀態。
- `controls=0` 只拿掉下面那條控制列，**暫停時那一整片標題與品牌照樣出現**，
  而代價是不能拖進度、沒有全螢幕鈕。一段是半局六到八分鐘，所以不用。
- 那些覆蓋物都是**固定像素高度**，所以播放器越小被擋的比例越高。在三欄網格裡
  一格只有兩百多像素高，等於蓋掉快一半 —— **唯一有效的做法是把播放器放大**，
  所以 `GameClips` 讓正在播的那一段展開成整列寬度。

`clipEmbedUrl()` 現在帶 `autoplay=1&rel=0&playsinline=1`，
`tests/unit/game-logic.test.ts` 的 `clipEmbedUrl` 那組守著（包含「不要再加
`modestbranding`」）。
