...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    ...
    public: {
      ...
      /**
       * 比賽進行中時，前台比賽頁重新抓資料的間隔（毫秒）。
       *
       * 計分板、比數與影片片段都是後台隨時在更新的，而比賽頁的 HTML 走
       * CDN 快取 —— 沒有輪詢的話，開著頁面的人要手動重新整理才看得到變化。
       *
       * **只有 `status: 'live'` 的比賽會輪詢**，而且分頁切到背景就暫停：
       * 已經結束的比賽不會再變，對著看不見的分頁發請求也沒有意義。
       *
       * 設成 `0` 就關閉輪詢。用環境變數 `NUXT_PUBLIC_GAME_POLL_MS` 可以
       * 不改程式碼調整。
       */
      gamePollMs: 120_000,
      /**
